...
public class DenyController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String repairer = (String) session.getAttribute("USER");
            String userID = request.getParameter("sender");
            FeedbackDTO dto = new FeedbackDTO(feedbackID, repairer);
            if (dao.reject(dto, userID)) {
                url = SUCCESS;
            } else {
                request.setAttribute("ERROR", "Reject Failed");
            }
        } catch (Exception e) {
            log("ERROR at DenyController: " + e.getMessage());
        } finally {
